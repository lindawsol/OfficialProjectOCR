...
input = '/Users/stephane.sol/Developer/OfficialProjectOCR/Data/Text/37.txt'
...
for idx, val in enumerate(page[1::]):
    ...
    ##TODO make the resub below greedy
    ...
    split3 = re.sub(r'([a-z]{1,2}\.(?!\s[a-z]{1,2}\.)|\(.*\)(?!\s[a-z]{1,2}\.)|\s(?![a-z]{1,2}\.)(?!\(.*\))(?!m\s)(?!pl\s)(?!f\s))',r'\1|', split2[0],1).split('|')
    ...
    try:
        ...
        english_sentence = split[1]
        french_sentence = split2[1]
        english_word = split3[1]
        french_word = split3[0]
        try:
            french_sent2 = split[2]
        except IndexError:
            french_sent2 = ''
        try:
            english_sent2 = split[3]
        except IndexError:
            english_sent2 = ''
    except IndexError:
            print('Error at {0}'.format(entry_key))
